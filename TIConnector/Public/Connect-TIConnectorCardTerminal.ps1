<#
.SYNOPSIS
    Connects one or more card terminals on a TI-Connector.

.DESCRIPTION
    Sends a connect request for the specified card terminal(s) via REST API.
...
.EXAMPLE
    Connect-TIConnectorCardTerminal -ComputerName "192.168.1.100" -Credential $cred -Id "373a533e-77eb-45a3-87ad-435a6b6826ad"

.EXAMPLE
    Get-TIConnectorCardTerminal -ComputerName "192.168.1.100" -Credential $cred -Name "KT" | Connect-TIConnectorCardTerminal
#>
function Connect-TIConnectorCardTerminal {
    ...
    process {
        ...
        for ($i = 0; $i -lt $Id.Count; $i++) {
            ...
            if ($PSCmdlet.ShouldProcess($targetName, "Connect card terminal")) {
                try {
                    Invoke-TIConnectorRequest -ComputerName $ComputerName -Credential $Credential -Request EnableConnectorCardTerminalConnection -PathParameters @{ CardTerminalID = $currentId } | Out-Null
                    Write-Verbose "Card terminal '$targetName' successfully connected."
                }
                catch {
