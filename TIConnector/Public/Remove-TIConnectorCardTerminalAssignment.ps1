<#
.SYNOPSIS
    Removes assignment of one or more card terminals on a TI-Connector.

.DESCRIPTION
    Sends an assignment removal request for the specified card terminal(s) via REST API.
...
.EXAMPLE
    Remove-TIConnectorCardTerminalAssignment -ComputerName "192.168.1.100" -Credential $cred -Id "373a533e-77eb-45a3-87ad-435a6b6826ad"

.EXAMPLE
    Get-TIConnectorCardTerminal -ComputerName "192.168.1.100" -Credential $cred -Name "KT" | Remove-TIConnectorCardTerminalAssignment
#>
function Remove-TIConnectorCardTerminalAssignment {
    ...
    process {
        ...
        for ($i = 0; $i -lt $Id.Count; $i++) {
            ...
            if ($PSCmdlet.ShouldProcess($targetName, "Remove assignment of card terminal")) {
                try {
                    Invoke-TIConnectorRequest -ComputerName $ComputerName -Credential $Credential -Request RemoveConnectorCardTerminalAssignment -PathParameters @{ CardTerminalID = $currentId } | Out-Null
                    Write-Verbose "Card terminal assignment '$targetName' successfully removed."
                }
                catch {
